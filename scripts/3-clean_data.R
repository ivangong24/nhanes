...
nhanes_master %<>%
  ...
  select(-c(
    lb2day.x.x, wtdrd1.x, smaquex.x, smq077.x, wtsaf2yr.y, wtsaf4yr.y,
...
nhanes_codebook_final <- nhanes_codebook %>%
  mutate_all(~ replace(., which(. %in% list(
    "wtsaf2yr.x", "wtsaf4yr.x", "lb2day.x",
...
for (a in 1:3) {
  ...
  for (i in 1:seq_len(valuelist[[a]])) {
...
nhanes_final <- full_join(nhanes_master, paqiaf_99to06, by = "seqn") %>%
  ...
  modify_at(vars(
    mcq240b, mcq240d, mcq240h, mcq240k, mcq240l, mcq240t, mcq240v, mcq240y,
    mcq240aa, mcq240dk
  ), as.numeric) %>%
...
test <- nhanes_final %>%
  mutate(
    mec16yr = case_when(
      sddsrvyr %in% c("NHANES 1999-2000 Public Release", "NHANES 2001-2002 Public Release") ~ 2 / 8 * wtmec4yr,
      sddsrvyr %notin% c("NHANES 1999-2000 Public Release", "NHANES 2001-2002 Public Release") ~ 1 / 8 * wtmec2yr
    ),
    int16yr = case_when(
      sddsrvyr %in% c("NHANES 1999-2000 Public Release", "NHANES 2001-2002 Public Release") ~ 2 / 8 * wtint4yr,
      sddsrvyr %notin% c("NHANES 1999-2000 Public Release", "NHANES 2001-2002 Public Release") ~ 1 / 8 * wtint2yr
    ),
    saf16yr = case_when(
      sddsrvyr %in% c("NHANES 1999-2000 Public Release", "NHANES 2001-2002 Public Release") ~ 2 / 8 * wtsaf4yr,
      sddsrvyr %notin% c("NHANES 1999-2000 Public Release", "NHANES 2001-2002 Public Release") ~ 1 / 8 * wtsaf2yr
    ),
    dr16yr = case_when(
      sddsrvyr %in% c("NHANES 1999-2000 Public Release", "NHANES 2001-2002 Public Release") ~ 2 / 8 * wtdr4yr,
    ...
    trmetwk = case_when(
      paq050u == 1 ~ 4 * pad080 * paq050q * 7 / 60,
    ...
    alcohol = case_when(
      alq100 == 1 ~ "Drink alcohol",
      alq100 == 2 ~ "Don't drink alcohol"
    ),
    htn = case_when(
      bpxsar >= 130 | bpxdar >= 80 ~ "With Hypertension",
      bpxsar < 130 & bpxdar < 80 ~ "Without hypertension"
    ),
    diab = case_when(
      (lbxglu >= 126 & phafsthr >= 12) | (lbxglu >= 200 & phafsthr < 12) |
